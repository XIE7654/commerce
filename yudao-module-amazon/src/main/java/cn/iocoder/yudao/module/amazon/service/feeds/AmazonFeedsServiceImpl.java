...
public class AmazonFeedsServiceImpl implements AmazonFeedsService {
    ...
    /** 取消尚未处理的 Feed。 */
    @Override public void cancelFeed(AmazonFeedIdReqVO request) { AmazonMarketplaceEnum m=marketplace(request.getCountryCode()); AmazonShopDO s=shop(request.getShopId()); amazonSellingPartnerClient.cancelFeed(URI.create(m.getEndpoint()+PATH+"/feeds/"+encodePath(request.getId())), amazonOAuthService.getSellerAccessToken(s.getId()),s.getId(),request.getCountryCode(),m.getMarketplaceId()); }
    ...
    private Map<String,Object> call(Long id,String cc,AmazonMarketplaceEnum m,String path,String op,String storage,Object body){ AmazonShopDO s=shop(id); String token=amazonOAuthService.getSellerAccessToken(s.getId()); URI u=URI.create(m.getEndpoint()+path); if(body==null) return amazonSellingPartnerClient.getFeeds(u,token,op,storage,s.getId(),cc,m.getMarketplaceId()); return amazonSellingPartnerClient.createFeed(u,token,body,op,storage,s.getId(),cc,m.getMarketplaceId()); }
