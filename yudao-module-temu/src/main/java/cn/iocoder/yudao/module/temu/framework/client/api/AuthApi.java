...
public class AuthApi {
    ...
    public AuthApi(TemuClient client) { this.client = client; }
    ...
    private <T> TemuApiResponse<T> response(String type, Map<String, ?> params, Class<T> resultType) {
        ...
        result.setResult(client.convert(raw.get("result"), resultType));
        return result;
    }
    private String text(tools.jackson.databind.JsonNode node, String field) {
        var value = node.get(field); return value == null || value.isNull() ? null : value.asString();
    }
}
